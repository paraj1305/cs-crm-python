...
from superadmin.models import Superadmin,Client,Employee,Project,ChangeRequest,Task,Board,Lead,Invoice,InvoiceItem,ClientImage,ProjectFile,EmployeeImage,LeadFile
import re
...
class SuperadminForm(forms.ModelForm):
    class Meta:
        model = Superadmin
        fields = ['email', 'company_name', 'bank', 'bank_no', 'SWIFT_CODE', 'IFSC_CODE', 'logo', 'website']
...
class ClientForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'since': forms.Select(choices=[(year, year) for year in range(1970, 2025)]),
