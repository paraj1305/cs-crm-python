...
def update_task(request, task_id):
    ...
    # Pre-select the project associated with the task
    selected_project = task.project
    employees = selected_project.employees.all() if selected_project else Employee.objects.none()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'attachment' in request.FILES:
                file = request.FILES['attachment']
                TaskFile.objects.create(task=updated_task, file=file)
            updated_task.save()
        ...
        form = TaskForm(instance=task, initial={'project': selected_project.id})
